...
def forecast_trend_peserta_sarimax(df,selected_kota: list[str], forecast_years: int = 5) -> pd.DataFrame:
    df_agg = df
    all_forecasts = []
...
def plot_forecast_line(df_forecast):
    fig = go.Figure()
    ...
    for kota in df_forecast['bps_nama_kabupaten_kota'].unique():
        ...
        # Tambah satu trace untuk garis penuh (actual+forecast)
...
#     # Ambil daftar unik kota dan warna continuous
#     kota_list = df_forecast['bps_nama_kabupaten_kota'].unique()
#     colors = px.colors.qualitative.G10
#     color_map = {kota: colors[i % len(colors)] for i, kota in enumerate(kota_list)}
...
#             mode='lines+markers',
#             name=kota,
#             line=dict(color=color_map[kota]),
#             marker=dict(color=color_map[kota])
#         ))

#         # Marker khusus untuk forecast
#         df_fc = df_kota[df_kota['tipe'] == 'forecast']
#         fig.add_trace(go.Scatter(
#             x=df_fc['tahun'],
#             y=df_fc['jumlah'],
...
#                         line=dict(width=2, color=color_map[kota]),
#                         color="white")
#         ))
...
#             y=pd.concat([df_fc['upper_ci'], df_fc['lower_ci'][::-1]]),
#             fill='toself',
#             fillcolor=color_map[kota].replace("rgb", "rgba").replace(")", ",0.05)"),
#             line=dict(color='rgba(255,255,255,0)'),
#             hoverinfo="skip",
#             name=f"{kota} CI"
#         ))
